...
abstract public class GraphView extends LinearLayout {
		...
		if (!ignoreViewport && viewportSize != 0) {
			...
			for (int i=0; i<graphSeries.size(); i++) {
				...
				for (int ii=0; ii<values.length; ii++)
					if (values[ii].getX() > highest)
						highest = values[ii].getX();
			}
			//Log.d("GraphView", "maxX="+highest);
			//if (highest == Integer.MIN_VALUE) highest = 0;
			return highest;
		}
	}
	...
	protected double getMinX(boolean ignoreViewport) {
		...
		if (!ignoreViewport && viewportSize != 0) {
			...
			for (int i=0; i<graphSeries.size(); i++) {
				GraphViewDataInterface[] values = _values(i);
				for (int ii=0; ii<values.length; ii++)
					if (values[ii].getX() < lowest)
						lowest = values[ii].getX();
			}
			//Log.d("GraphView", "minX="+lowest);
	...
	protected double getMinY() {
		...
		//if (smallest == Integer.MAX_VALUE) smallest = 0;
		return smallest;
	}
