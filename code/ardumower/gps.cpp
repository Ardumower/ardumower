...
void GPS::get_position(long * latitude, long * longitude, unsigned long * fix_age) const
{
    if (latitude) {
        *latitude = _latitude;
    }
...
void GPS::get_datetime(unsigned long * date, unsigned long * time, unsigned long * age) const
{
    if (date) {
        *date = _date;
    }
...
void GPS::f_get_position(float * latitude, float * longitude, unsigned long * fix_age) const
{
    long lat, lon;
...
void GPS::crack_datetime(int * year, byte * month, byte * day,
                         byte * hour, byte * minute, byte * second, byte * hundredths, unsigned long * age) const
{
...
float GPS::f_altitude() const
{
    return _altitude == GPS_INVALID_ALTITUDE ? GPS_INVALID_F_ALTITUDE : _altitude / 100.0;
}

float GPS::f_course() const
{
    return _course == GPS_INVALID_ANGLE ? GPS_INVALID_F_ANGLE : _course / 100.0;
}

float GPS::f_speed_knots() const
{
    return _speed == GPS_INVALID_SPEED ? GPS_INVALID_F_SPEED : _speed / 100.0;
}

float GPS::f_speed_mph() const
{
    float sk = f_speed_knots();
    return sk == GPS_INVALID_F_SPEED ? GPS_INVALID_F_SPEED : MPH_PER_KNOT * f_speed_knots();
}

float GPS::f_speed_mps() const
{
    float sk = f_speed_knots();
    return sk == GPS_INVALID_F_SPEED ? GPS_INVALID_F_SPEED : MPS_PER_KNOT * f_speed_knots();
}

float GPS::f_speed_kmph() const
{
    float sk = f_speed_knots();
    return sk == GPS_INVALID_F_SPEED ? GPS_INVALID_F_SPEED : KMH_PER_KNOT * f_speed_knots();
}
