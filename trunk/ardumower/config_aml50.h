// Ambrogio L50 robot mower configuration (Ardumower electronics)
...
// ------ pins---------------------------------------
#define pinMotorLeftPWM 5          // IN2 left motor pins
#define pinMotorLeftDir 31         // IN1        
#define pinMotorLeftSense A1
#define pinMotorRightPWM  3        // IN2 right motor pins
#define pinMotorRightDir 33        // IN1
...
int motorCurrentMax     = 25;    // motor wheel max current (8-bit ADC:0..255 , 10-bit ADC:0..1023)
int motorSenseRightZero = 0;     // motor right sense zero point 
int motorSenseLeftZero  = 0;     // motor left sense zero point
...
double motorBiDirSpeedRatio1 = 0.55;   // bidir mow pattern speed ratio 1
double motorBiDirSpeedRatio2 = 0.92;   // bidir mow pattern speed ratio 2
// ------ mower motor -------------------------------
double motorMowAccel   = 0.02;   // motor mower acceleration (warning: do not set too high)
int motorMowSpeedMax   = 255;    // motor mower max PWM
int motorMowCurrentMax = 60;     // motor mower max current
char motorMowModulate  = 1;      // motor mower cutter modulation?
int motorMowRPM        = 3300;   // motor mower RPM (only for cutter modulation)
int motorMowSenseZero  = 0;      // motor mower sense zero point
...
void config(){
  ...
  /*  Alexander's test code - will be removed
  imu.init();  
  int i=0;
  while (true){
    float ypr[3];
...
int readSensor(char type){
  ...
  switch (type) {
    ...
    // reverse direction        
    case SEN_MOTOR_RIGHT: return(ADCMan.read(pinMotorLeftSense)-motorSenseRightZero); break;
    case SEN_MOTOR_LEFT: return(ADCMan.read(pinMotorRightSense)-motorSenseLeftZero); break;    
    //case SEN_MOTOR_RIGHT: return(analogRead(pinMotorLeftSense)-motorSenseRightZero); break;
    //case SEN_MOTOR_LEFT: return(analogRead(pinMotorRightSense)-motorSenseLeftZero); break;    
    // normal direction
...
void setActuator(char type, int value){
  switch (type){
    case ACT_MOTOR_MOW: analogWrite(pinMotorMow, value); break;
    // normal direction
