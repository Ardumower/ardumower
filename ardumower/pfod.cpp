...
void sendBatteryMenu(boolean update){
  ...
  sendSlider("j05", F("Calibrate battery V "), batVoltage, "", 0.01, 30, 0);   
  sendSlider("j02", F("Go home if below"), batGoHomeIfBelow, "", 0.1, batFull, (batFull*0.72));  // for Sony Konion cells 4.2V * 0,72= 3.024V which is pretty safe to use 
  sendSlider("j03", F("Switch off if below"), batSwitchOffIfBelow, "", 0.1, batFull, (batFull*0.72));  
...
void sendOdometryMenu(boolean update){
  ...
  Serial2.print(", "); 
  Serial2.println(odometryRight); 
  Serial2.println(F("|l03~RPM Motor l, r "));
  Serial2.print(motorLeftRpm);
  Serial2.print(", "); 
  Serial2.println(motorRightRpm);
  sendSlider("l04", F("Ticks per one full revolution"), odometryTicksPerRevolution, "", 1, 2000);       
...
void processOdometryMenu(String pfodCmd){      
  if (pfodCmd == "l00") odometryUse = !odometryUse;
    ...
    else if (pfodCmd.startsWith("l04")) processSlider(pfodCmd, odometryTicksPerRevolution, 1);    
