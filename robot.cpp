...
void Robot::readSensors() {
  ...
  if (millis() >= nextTimeMotorSense) {
    ...
    motor2MowSenseCurrent = motor2MowSenseCurrent * (1.0 - accel) + ((double)motor2MowSenseADC) * motor2MowSenseScale * accel;
  ...
  if (millis() >= nextTimeBattery) {
    // read battery
    nextTimeBattery = millis() + 500;       // changed from 100 to 500
    ...
    // low-pass filter
    double accel = 0.05;  //changed from 0.01 to 0.05
    //double accel = 1.0;
...
void Robot::checkCurrent() {
  ...
  if ((motor1MowSenseCounter >= 30) || (motor2MowSenseCounter >= 30)) { //ignore motorMowPower for 3 seconds
    motorMowEnable = false;
...
void Robot::loop()  {
  ...
  switch (stateCurr) {
    ...
    case STATE_STATION: //Original code
      // waiting until auto-start by user or timer triggered
      if (batMonitor) {
        if (chgVoltage > 5.0) {
          //if (batVoltage < startChargingIfBelow && (millis() - stateStartTime > 2000)) { //Original code
          if (batVoltage < startChargingIfBelow) { //Modified code, read the battery voltage immediatly
